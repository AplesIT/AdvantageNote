...
import com.production.advangenote.helpers.notifications.NotificationChannels.NotificationChannelNames;
...
public class DataBackupIntentService extends IntentService implements OnAttachingFileListener {
  private final static Logger logger = LoggerFactory.getLogger("DataBackupIntentService.class");
...
//    }
  ...
  private void importDataFromSpringpad(Intent intent, NotificationsHelper mNotificationsHelper) {
    new SpringImportHelper(AdvantageNotes.getAppContext())
        .importDataFromSpringpad(intent, mNotificationsHelper);
...
//        }
  ...
  private void createNotification(Intent intent, Context mContext, String title, String message,
    ...
    Intent intentLaunch;
    if (com.production.advangenote.async.DataBackupIntentService.ACTION_DATA_IMPORT.equals(intent.getAction())
  ...
  private void resetReminders() {
    logger.info("Resettings reminders");
