...
public class SpringImportHelper {
  ...
  private final static Logger logger = LoggerFactory.getLogger("SpringImportHelper.class");
  ...
  public synchronized void importDataFromSpringpad(Intent intent,
    ...
    for (SpringpadElement springpadElement : importer.getNotebooks()) {
      ...
      DAOSQL.getInstance().updateCategory(cat);
    ...
    DAOSQL.getInstance().updateCategory(defaulCategory);
    ...
    for (SpringpadElement springpadElement : importer.getNotes()) {
      ...
      if (!TextUtils.isEmpty(address)) {
        try {
          ...
          logger.info("An error occurred trying to resolve address to coords during Springpad " +
              "import");
        }
      ...
      if (!TextUtils.isEmpty(image)) {
        try {
          ...
          logger.info("Error retrieving Springpad online image");
        }
        if (mAttachment != null) {
    ...
    try {
      ...
      logger.info("Springpad import temp files not deleted");
    }
  }
