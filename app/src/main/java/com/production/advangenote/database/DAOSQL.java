package com.production.advangenote.database;
...
import com.production.advangenote.async.upgrade.UpgradeProcessor;
import com.production.advangenote.exception.DatabaseException;
...
import com.production.advangenote.models.Stats;
import com.production.advangenote.models.Tag;
import com.production.advangenote.utils.AssetUtils;
...
import static com.production.advangenote.utils.ConstantsBase.TIMESTAMP_UNIX_EPOCH;
...
{
    private final static Logger logger = LoggerFactory.getLogger("DAOSQL.class");
    // Database name
    ...
    public static synchronized DAOSQL getInstance() {
        return getInstance(AdvantageNotes.getAppContext());
    }


    public static synchronized DAOSQL getInstance(Context context) {
        if (instance == null) {
            instance = new DAOSQL(context);
        }
        return instance;
    }


    public static synchronized DAOSQL getInstance(boolean forcedNewInstance) {
        if (instance == null || forcedNewInstance) {
            Context context = (instance == null || instance.mContext == null) ? AdvantageNotes.getAppContext()
                    : instance.mContext;
            instance = new DAOSQL(context);
        }
        return instance;
    ...
    private DAOSQL(Context mContext) {
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        this.db = db;
        logger.info("Upgrading database version from " + oldVersion + " to " + newVersion);
    ...
    public Note updateNote(Note note, boolean updateLastModification) {
        ...
        logger.info("Updated note titled '" + note.getTitle() + "'");
    ...
    private void execSqlFile(String sqlFile, SQLiteDatabase db) throws SQLException, IOException {
        ...
        for (String sqlInstruction : Sqlutils
                .parseSqlFile(SQL_DIR + "/" + sqlFile, mContext.getAssets())) {
            logger.info("    sql: {}" + sqlInstruction);
            try {
                db.execSQL(sqlInstruction);
            } catch (Exception e) {
                logger.error("Error executing command: " + sqlInstruction, e);
            }
        }
    }
    ...
    public List<Note> getNotes(String whereCondition, boolean order) {
        ...
        logger.info("Query: Retrieval finished!");
        return noteList;
    }
