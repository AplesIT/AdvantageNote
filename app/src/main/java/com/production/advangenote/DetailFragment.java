...
import com.production.advangenote.async.AttachmentTask;
...
import com.production.advangenote.models.adapters.AttachmentAdapter;
import com.production.advangenote.models.adapters.CategoryRecyclerViewAdapter;
import com.production.advangenote.models.adapters.PlacesAutoCompleteAdapter;
import com.production.advangenote.models.listeners.OnAttachingFileListener;
import com.production.advangenote.models.listeners.OnGeoUtilResultListener;
...
import com.production.advangenote.models.listeners.RecyclerViewItemClickSupport;
import com.production.advangenote.models.views.ExpandableHeightGridView;
import com.production.advangenote.utils.AlphaManager;
import com.production.advangenote.utils.BitmapHelper;
import com.production.advangenote.utils.ConnectionManager;
import com.production.advangenote.utils.Display;
import com.production.advangenote.utils.FileHelper;
import com.production.advangenote.utils.FileProviderHelper;
import com.production.advangenote.utils.GeocodeHelper;
import com.production.advangenote.utils.IntentChecker;
import com.production.advangenote.utils.KeyboardUtils;
...
import static com.production.advangenote.BaseActivity.TRANSITION_HORIZONTAL;
import static com.production.advangenote.BaseActivity.TRANSITION_VERTICAL;
import static com.production.advangenote.MainActivity.FRAGMENT_DETAIL_TAG;
import static com.production.advangenote.utils.ConstantsBase.ACTION_DISMISS;
import static com.production.advangenote.utils.ConstantsBase.ACTION_FAB_TAKE_PHOTO;
import static com.production.advangenote.utils.ConstantsBase.ACTION_MERGE;
...
import static com.production.advangenote.utils.ConstantsBase.INTENT_GOOGLE_NOW;
...
import static com.production.advangenote.utils.ConstantsBase.MIME_TYPE_IMAGE_EXT;
import static com.production.advangenote.utils.ConstantsBase.MIME_TYPE_SKETCH;
import static com.production.advangenote.utils.ConstantsBase.MIME_TYPE_SKETCH_EXT;
import static com.production.advangenote.utils.ConstantsBase.MIME_TYPE_VIDEO;
import static com.production.advangenote.utils.ConstantsBase.MIME_TYPE_VIDEO_EXT;
import static com.production.advangenote.utils.ConstantsBase.PREF_ATTACHMENTS_ON_BOTTOM;
import static com.production.advangenote.utils.ConstantsBase.PREF_AUTO_LOCATION;
import static com.production.advangenote.utils.ConstantsBase.PREF_COLORS_APP_DEFAULT;
import static com.production.advangenote.utils.ConstantsBase.PREF_KEEP_CHECKED;
import static com.production.advangenote.utils.ConstantsBase.PREF_KEEP_CHECKMARKS;
...
import static com.production.advangenote.utils.ConstantsBase.SWIPE_MARGIN;
import static com.production.advangenote.utils.ConstantsBase.SWIPE_OFFSET;
import static com.production.advangenote.utils.ConstantsBase.THUMBNAIL_SIZE;
...
public class DetailFragment extends BaseFragment implements OnReminderPickedListener,
  ...
  private Style exitCroutStyleOn = StyleOn.CONFIRM;
  ...
  private final static Logger logger = LoggerFactory.getLogger("DetailFragment.class");
  ...
  public void onActivityCreated(Bundle savedInstanceState) {
    ...
    prefs = mainActivity.sharedPreferences;
  ...
  private void handleIntents() {
    ...
    if (IntentChecker.checkAction(i, ACTION_SHORTCUT, ACTION_NOTIFICATION_CLICK)) {
      ...
      noteOriginal = DAOSQL.getInstance().getNote(i.getLongExtra(INTENT_KEY, 0));
    ...
    if (IntentChecker.checkAction(i, ACTION_WIDGET, ACTION_WIDGET_TAKE_PHOTO)) {
      ...
      if (i.hasExtra(INTENT_WIDGET)) {
        ...
        if (categoryId != null) {
          Category category;
          try {
            category = DAOSQL.getInstance().getCategory(parseLong(categoryId));
            noteTmp = new Note();
            noteTmp.setCategory(category);
          } catch (NumberFormatException e) {
            logger.info("Category with not-numeric value!", e);
          }
        }
      }
    ...
    binding.fragmentDetailContent.reminderLayout.setOnLongClickListener(v -> {
      MaterialDialog dialog = new MaterialDialog.Builder(mainActivity)
          ...
          .onPositive((dialog1, which) -> {
            ReminderHelper.removeReminder(AdvantageNotes.getAppContext(), noteTmp);
    ...
    mGridView.setOnItemClickListener((parent, v, position, id) -> {
      ...
      if (MIME_TYPE_FILES.equals(attachment.getMime_type())) {
        ...
        if (IntentChecker
          ...
          mainActivity.showMessage(R.string.feature_not_available_on_this_device, StyleOn.WARN);
        }
  ...
  private void performAttachmentAction(int attachmentPosition, int i) {
    switch (getResources().getStringArray(R.array.attachments_actions_values)[i]) {
      case "share":
        ...
        shareIntent
            .setType(StorageHelper.getMimeType(AdvantageNotes.getAppContext(), attachment.getUri()));
        shareIntent.putExtra(Intent.EXTRA_STREAM, FileProviderHelper.getShareableUri(attachment));
        if (IntentChecker.isAvailable(AdvantageNotes.getAppContext(), shareIntent, null)) {
      ...
      case "edit":
        ...
        break;
      default:
        logger.info("No action available");
    }
  }
...
//					((View)event.getLocalState()).setVisibility(View.VISIBLE);
  ...
  public void onLocationRetrieved(Location location) {
    if (location == null) {
      mainActivity.showMessage(R.string.location_not_found, StyleOn.ALERT);
    }
    if (location != null) {
  ...
  private boolean goHome() {
    ...
    if (!afterSavedReturnsToList) {
      ...
      if (!TextUtils.isEmpty(exitMessage) && exitCroutStyleOn != null) {
        mainActivity.showMessage(exitMessage, exitCroutStyleOn);
      }
  ...
  public boolean onOptionsItemSelected(MenuItem item) {
    ...
    switch (item.getItemId()) {
      ...
      case R.id.menu_note_info:
        ...
        break;
      default:
       logger.info("Invalid menu option selected");
    }
  ...
  private void toggleChecklist2(final boolean keepChecked, final boolean showChecks) {
    ...
    try {
     ...
     logger.info("Error switching checklist view", e);
    }
  ...
  private void categorizeNote() {
    ...
    final List<Category> categories = Observable.from(DAOSQL.getInstance().getCategories())
        .map(category -> {
  ...
  private void takePhoto() {
    ...
    if (!IntentChecker
      ...
      mainActivity.showMessage(R.string.feature_not_available_on_this_device, StyleOn.ALERT);
    ...
    if (f == null) {
      mainActivity.showMessage(R.string.error, StyleOn.ALERT);
      return;
    }
    ...
    if (resultCode == Activity.RESULT_OK) {
      switch (requestCode) {
        ...
        case SKETCH:
          ...
          break;
        case CATEGORY:
          mainActivity.showMessage(R.string.category_saved, StyleOn.CONFIRM);
          Category category = intent.getParcelableExtra("category");
          noteTmp.setCategory(category);
          setTagMarkerColor(category);
          break;
        case DETAIL:
          mainActivity.showMessage(R.string.note_updated, StyleOn.CONFIRM);
          break;
        default:
          logger.info("Wrong element choosen: " + requestCode);
      }
    }
  }
  ...
  private void archiveNote(boolean archive) {
    ...
    exitCroutStyleOn = archive ? StyleOn.WARN : StyleOn.INFO;
    saveNote(this);
  }
  ...
  private void trashNote(boolean trash) {
    ...
    exitCroutStyleOn = trash ? StyleOn.WARN : StyleOn.INFO;
    if (trash) {
      ShortcutHelper.removeShortcut(AdvantageNotes.getAppContext(), noteTmp);
      ReminderHelper.removeReminder(AdvantageNotes.getAppContext(), noteTmp);
    } else {
  ...
  private void deleteNote() {
    new MaterialDialog.Builder(mainActivity)
        ...
        .onPositive((dialog, which) -> {
          ...
          logger.info("Deleted note with ID '" + noteTmp.get_id() + "'");
          mainActivity.showMessage(R.string.note_deleted, StyleOn.ALERT);
          goHome();
        }).build().show();
  }
  ...
  public void saveAndExit(OnNoteSaved mOnNoteSaved) {
    if (isAdded()) {
      ...
      exitCroutStyleOn = StyleOn.CONFIRM;
      goBack = true;
  ...
  void saveNote(OnNoteSaved mOnNoteSaved) {
    ...
    if (goBack && TextUtils.isEmpty(noteTmp.getTitle()) && TextUtils.isEmpty(noteTmp.getContent())
      ...
      logger.info("Empty note not saved");
      exitMessage = getString(R.string.empty_note_not_saved);
      exitCroutStyleOn = StyleOn.INFO;
      goHome();
      return;
    }
  ...
  public void onNoteSaved(Note noteSaved) {
    MainActivity.notifyAppWidgets(AdvantageNotes.getAppContext());
    if (!activityPausing) {
  ...
  private void lockNote() {
    logger.info("Locking or unlocking note " + note.get_id());
  ...
  private void lockUnlock() {
    ...
    if (prefs.getString(PREF_PASSWORD, null) == null) {
      mainActivity.showMessage(R.string.password_not_set, StyleOn.WARN);
      return;
    }
    mainActivity.showMessage(R.string.save_note_to_lock_it, StyleOn.INFO);
  ...
  private void pinNote() {
    ...
    notificationsHelper
        .createOngoingNotification(NotificationChannels.NotificationChannelNames.PINNED, R.drawable.ic_stat_notification,
  ...
  private void addShortcut() {
    ShortcutHelper.addShortcut(AdvantageNotes.getAppContext(), noteTmp);
    mainActivity.showMessage(R.string.shortcut_added, StyleOn.INFO);
  }
  ...
  public boolean onTouch(View v, MotionEvent event) {
    ...
    switch (event.getAction()) {
      ...
      case MotionEvent.ACTION_DOWN:
        logger.info("MotionEvent.ACTION_DOWN");
        int w;
      ...
      case MotionEvent.ACTION_UP:
       logger.info("MotionEvent.ACTION_UP");
        if (swiping) {
          swiping = false;
        }
        break;
      ...
      case MotionEvent.ACTION_MOVE:
        if (swiping) {
         logger.info("MotionEvent.ACTION_MOVE at position " + x + ", " + y);
      ...
      default:
       logger.info("Wrong element choosen: " + event.getAction());
    }
  ...
  public void onAttachingFileErrorOccurred(Attachment mAttachment) {
    mainActivity.showMessage(R.string.error_saving_attachments, StyleOn.ALERT);
  ...
  public void onRecurrenceReminderPicked(String recurrenceRule) {
    ...
    if (!TextUtils.isEmpty(recurrenceRule)) {
      logger.info("Recurrent reminder set: " + recurrenceRule);
  ...
  private void addTags() {
    ...
    if (tags.isEmpty()) {
      mainActivity.showMessage(R.string.no_tags_created, StyleOn.WARN);
      return;
    }
  ...
  private static class OnGeoUtilResultListenerImpl implements OnGeoUtilResultListener {
    ...
    @Override
    public void onLocationUnavailable() {
      mainActivityWeakReference.get().showMessage(R.string.location_not_found, StyleOn.ALERT);
    }
    ...
    public void onClick(View v) {
      ...
      switch (v.getId()) {
        ...
        case R.id.pushbullet:
          ...
          break;
        default:
          logger.info("Wrong element choosen: " + v.getId());
      }
      if (!isRecording) {
